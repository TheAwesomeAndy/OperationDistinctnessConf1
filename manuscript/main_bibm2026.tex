...
\title{On the Non-Invariance of Robustness Metrics: Baseline-Floor and Imputation Effects in Perturbed EEG Representations}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_reservoir_dynamics_inscope.pdf}
\caption{Reservoir-derived objects for the three affective conditions: (a)~LIF spike raster (shaded $=$ the $t\in[10,70]$ analysis window); (b)~membrane response; (c)~\BSC{} embedding in its first two principal components.}
\label{fig:resdyn}
\end{figure}
...
\label{sec:clean}
Table~\ref{tab:clean} reports clean three-class performance, and Fig.~\ref{fig:overview} shows the full accuracy landscape under clean, amplitude-noise, and (naive, zero-imputed) channel-dropout conditions. The trained EEGNet is the most accurate representation (BA $0.711$); among the fixed encoders, ERP-window features lead (A0$_e$, $0.616$), above band-power (A0, $0.485$) and the reservoir embedding (A1, $0.463$). A label-permutation null gives BA $0.338\pm0.022$, so every configuration encodes genuine signal (all exceed the null by more than five standard deviations) while the absolute clean separability of the fixed encoders remains modest.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{ana03_robustness_inscope.pdf}
\caption{Accuracy landscape under clean, $5$~dB amplitude-noise, and $30\%$ channel-dropout conditions (mean, 95\% CI over 25 folds; dropout uses the naive zero-imputation). EEGNet is most accurate throughout; among the fixed encoders the reservoir is least accurate but least degraded, ERP-window strongest on clean data. Sec.~\ref{sec:perturb} shows that the dropout ordering here is an evaluation artifact.}
\label{fig:overview}
\end{figure}
...
\emph{Generality.} The mean-minus-zero gap predicted by Eq.~\eqref{eq:noninv} is $+0.034$ (ERP-window), $+0.023$ (band-power), and only $+0.005$ (reservoir) at $30\%$ dropout: it tracks feature non-centeredness and vanishes for the PCA-centered embedding, as predicted. The effect is not specific to mean imputation or to one dropout rate (Fig.~\ref{fig:impute}). Across $10$--$50\%$ dropout every in-distribution fill (mean, kNN, spatial) raises the non-centered encoders---by up to $0.11$~BA for ERP-window under kNN---while leaving the reservoir within $0.04$~BA of its zero-imputation value. Consequently the reservoir's naive lead does not survive: at $30\%$ dropout ERP-window significantly exceeds the reservoir under kNN ($0.562$ vs.\ $0.491$, $p{=}2\times10^{-5}$) and spatial fills ($0.516$ vs.\ $0.462$, $p{=}10^{-3}$) and matches it under mean imputation ($p{=}0.06$), whereas under zero-imputation the two are statistically tied ($p{=}0.8$).
...
\emph{Other controls.} A dimensionality-matched random projection of band-power (2176-D, five seeds) gives clean BA $0.501\pm0.002$ but collapses to $0.399\pm0.008$ at $5$~dB, tracking band-power rather than the reservoir, so the reservoir's robustness is not a dimensionality effect.
...
\emph{Input-level check (reservoir).} Recomputing the embedding from corrupted \emph{signals} (perturbation before extraction, as for EEGNet) leaves it stable under amplitude noise and dropout (BA $0.449$, $0.431$ vs.\ $0.465$ clean) but sensitive to temporal jitter ($0.422$ at $\pm50$~ms), consistent with its dependence on input timing.
...
Several boundaries remain. The primary cohort is a single trial-averaged affective-ERP dataset; the imputation confound, however, follows from a deterministic property of standardized readouts (Eq.~\eqref{eq:noninv}) rather than from the data, and it held across three in-distribution imputation rules, five dropout rates, and an independent affective dataset (DEAP; Sec.~\ref{sec:deap}). DEAP is continuous video-elicited affect scored with band-power under a within-subject protocol, so paradigm-matched multi-cohort ERP validation remains valuable, but the confound's generality across datasets, tasks, and montages is now demonstrated. EEGNet is perturbed at its raw input while the fixed-feature comparisons are feature-level, so the cross-family comparison is approximate, though the input-level reservoir check (Sec.~\ref{sec:perturb}) and the augmentation analysis bracket this gap. We study amplitude noise and channel dropout; adversarial attacks and montage shifts are future work; graph topology and structure--function coupling are out of scope.
