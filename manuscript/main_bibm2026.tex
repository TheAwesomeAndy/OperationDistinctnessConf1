...
\title{Spiking Reservoir Encoding of Affective EEG: Perturbations and Invariances}
...
This paper evaluates whether channel-dropout robustness rankings for affective-ERP representations are stable under reasonable changes to the scoring protocol. We compare spectral band-power, ERP-window amplitudes, a fixed leaky integrate-and-fire (LIF) reservoir summarized by six post-onset spike-count bins (\BSC{}) and compressed for a linear readout, and a trained EEGNet (Fig.~\ref{fig:pipeline}). The reservoir is not treated as a proposed winner; it is used as a fixed reference whose embedding is nearly zero-centered and whose dynamics are not retrained under perturbation. This encoder is the spiking-reservoir embedding component of ARSPI-Net (Affective Reservoir-Spike Processing and Inference Network), a broader neuromorphic reservoir--graph framework for affective EEG~\cite{lane2023arspi,lane2024arspi}; its graph-topological, structure--function coupling, and closed-loop components are outside this submission. The contributions are:
\begin{enumerate}
...
Spiking and deep networks are now widely applied to EEG \cite{gong2023sglnet,lawhern2018eegnet,craik2019deep}. Much of this literature still emphasizes endpoint accuracy, while preprocessing and acquisition choices receive less systematic treatment \cite{delpup2025preprocessing,kornblith2019similarity}. Prior ARSPI-Net work developed neuromorphic reservoir features for affective EEG \cite{lane2023arspi,lane2024arspi}; the present study isolates the perturbation and scoring-artifact behavior of that reservoir encoding.
