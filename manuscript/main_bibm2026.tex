...
\caption{Subject-level channel-dropout results (feature-coordinate experiment). (a)~Balanced accuracy versus channels removed, under $k$NN fill (solid) and zero fill (dashed). The centered reservoir is nearly flat across fills, whereas ERP-window and band-power gain from the in-distribution fill; $k$NN-filled ERP-window leads at every rate. (b)~Paired subject-level difference ERP-window minus reservoir at $30\%$ dropout, with $95\%$ CIs. The difference is within noise under zero fill and its CI excludes zero under every in-distribution fill, the inference behind Proposition~1.}
\label{fig:impute}
\end{figure}
