...
\section{Conclusion}
Channel-dropout robustness rankings of affective-ERP representations are confounded by the accuracy floor, by zero-imputation of dropped channels, and---for trained models---by missing augmentation. Using a fixed, training-free reservoir as an accuracy- and imputation-invariant reference, correcting these confounds reverses the naive ranking, and the confound replicates on the independent DEAP dataset. We recommend reporting above-chance retention, the imputation rule, and the augmentation state in EEG robustness studies. Future work extends the audit to multi-cohort ERP data.
