...
Robustness to noise, artifacts, and electrode loss is increasingly studied. Dynamic spatial filtering handles missing channels with a learned attention front end \cite{banville2022dsf}, and channel-dropout or amplitude-noise augmentation hardens trained models \cite{rommel2022augment}. A recent EEG foundation-model benchmark finds that apparent channel-dropout fragility can depend on whether dropped channels are removed or zero-padded \cite{sirca2026beyond}. We build on that point by separating two scoring effects, raw-retention inflation and zero-fill bias, and using the training-free reservoir as a coordinate-independent reference.
...
\end{align}
where $v_t$ is the membrane potential and $z_t$ the binary spike vector; on each spike the potential is reset by subtracting $\vartheta$ and floored at zero ($r_t$). The weights $W_{\mathrm{in}}$ and $W$ are drawn once from a fixed Xavier-uniform law (seed $42$), $W$ is rescaled to $\rho=0.9$, and $\vartheta=0.5$. The reservoir is fixed during extraction; only the readout is trained.
